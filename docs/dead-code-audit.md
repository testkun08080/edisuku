...
| `sec_code_latest_periods` | **deprecated**（`company_metrics` で代替、テーブル DROP は未実施） |
...
| `raw_files_index`, `pipeline_runs` | 旧パイプライン残骸 | staging **1410 / 4 行**。R2 `object_key`（`raw/annual/.../*.json`）と `corpus-seed` / 旧 `daily-refresh` ログ。**現リポに書き込みコードなし**。DROP は要相談 |
| `daily_metrics` | seed のみ | 監視未実装。DROP は要相談 |
| `sec_code_latest_periods` | deprecated | 参照削除。テーブル DROP は要相談 |
